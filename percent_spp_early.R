...
ggplot(trmt_percent_graph, aes(Elevation, mean, color = Event))+
  ...
  geom_line()+
  theme_bw()+
  labs(y = "% Species Early", x = "Elevation (m)")
...
Model1 <- lm(percent_advanced ~ Site + Event, data = yr_percent)
...
Model2 <- lm(percent_advanced ~ Site + Event, data = trmt_percent)
Anova(Model2)
...
#test rcb
SumStats <- summarise(group_by(yr_percent, Site),
                      n = n(),
                      mean = mean(percent_advanced), sd = sd(percent_advanced),
                      SE = sd/sqrt(n))

Model3 <- lm(percent_advanced ~ Site + Event, data = yr_percent) 
Anova(Model3, type = 3)
